...
def prepare_callbacks(config_path, params_path):
    ...
    #  for each epoch Logging the loss, validation loss, accuracy and validation of models during training
    tensorboard_log_dir = os.path.join(artifacts_dir, 
    artifacts["TENSORBOARD_ROOT_LOG_DIR"])
    # Saving th snapshot of the updated models in checkpoint_dir during training
    checkpoint_dir = os.path.join(artifacts_dir, artifacts["CHECKPOINT_DIR"])
    # call back dir where all teh callbacks are being stored
